...
#### `create_dlt_message_with_file_header(buffer_ptr, buffer_len) -> i32`
DLTファイルヘッダ (`DLT\x01`) 付きのログメッセージを生成します。

ファイルヘッダはDLTメッセージをファイルに保存する際に使用される4バイトのマーカーです。

**パラメータ:**
- `buffer_ptr`: 出力バッファへのポインタ
- `buffer_len`: バッファサイズ (最低104バイト)

**戻り値:**
- 正の値: 生成されたメッセージサイズ（ファイルヘッダ4バイト含む）
- 負の値: エラーコード

**メッセージ構造:**
```
[DLT\x01 (4B)] [DLS\x01 (4B)] [標準ヘッダ] [拡張ヘッダ] [ペイロード]
 ファイルヘッダ  シリアルヘッダ
```
...
// ファイルヘッダ付きメッセージの生成
const size2 = wasmModule.create_dlt_message_with_file_header(bufferPtr, 256);
if (size2 > 0) {
    const message = new Uint8Array(wasmModule.memory.buffer, bufferPtr, size2);
    // 先頭4バイトが DLT\x01 (44 4c 54 01) であることを確認
    console.log('ファイルヘッダ付き:', Array.from(message).map(b => b.toString(16).padStart(2, '0')).join(''));
}
```
...
ECU IDを32ビット整数で取得します。ファイルヘッダ・シリアルヘッダを自動検出してスキップします。

#### `get_app_id(buffer_ptr, buffer_len) -> u32`
アプリケーションIDを32ビット整数で取得します。ファイルヘッダ・シリアルヘッダを自動検出してスキップします。

#### `get_context_id(buffer_ptr, buffer_len) -> u32`
コンテキストIDを32ビット整数で取得します。ファイルヘッダ・シリアルヘッダを自動検出してスキップします。

### ファイルヘッダ検出

#### `get_has_file_header(buffer_ptr, buffer_len) -> u8`
バッファがDLTファイルヘッダ (`DLT\x01`) で始まるかチェックします。
...
console.log('ファイルヘッダ:', hasFileHeader ? 'あり' : 'なし');
```
...
以下のHTMLページでブラウザ上からWASM APIをテストできます：

| ページ | 説明 |
|--------|------|
| `wasm_test.html` | 基本的なメッセージ生成・解析（ファイルヘッダ付き生成対応） |
| `test_messages.html` | 複数メッセージの一括Hex解析（ファイルヘッダ自動検出） |
| `wasm_service_test.html` | ログ・サービスメッセージ生成/解析（ファイルヘッダオプション付き） |
| `simple_demo.html` | 日本語デモ（ファイルヘッダ付きメッセージ生成対応） |
| `dlt_stream.html` | リアルタイムWebSocketストリームビューア（ファイルヘッダ検出対応） |
...
- DLTファイルヘッダ (`DLT\x01`) の自動検出・スキップに対応
- 手動バイト解析ではなく、テスト済みの公式パーサーで信頼性向上

### R19-11構造体の活用
解析時に以下のr19-11構造体を使用:
- `DltMessage`: 完全な解析済みメッセージ構造（`has_file_header`フィールド含む）
...
DLTファイルヘッダ (`DLT\x01` = `0x44 0x4C 0x54 0x01`) は、DLTメッセージをファイルに保存する際に使用されるオプションの4バイトマーカーです。シリアルヘッダ (`DLS\x01`) とは異なります。

**メッセージ構造（ファイルヘッダあり）:**
```
[44 4C 54 01]  [44 4C 53 01]  [標準ヘッダ (4B)] [追加フィールド] [拡張ヘッダ (10B)] [ペイロード]
  DLT\x01        DLS\x01
 ファイルヘッダ  シリアルヘッダ
```
...
**生成方法:**
```javascript
// 方法1: create_dlt_message_with_file_header() を使用
const size = wasmModule.create_dlt_message_with_file_header(bufferPtr, 256);

// 方法2: generate_log_message() の config[15] に 1 をセット
const cfgPtr = wasmModule.allocate(24);
const m = new Uint8Array(wasmModule.memory.buffer);
m[cfgPtr + 15] = 1;  // ファイルヘッダを有効化
```

**検出方法:**
```javascript
const hasFileHeader = wasmModule.get_has_file_header(bufferPtr, msgLen) === 1;
```
